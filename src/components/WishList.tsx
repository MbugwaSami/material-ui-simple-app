...
const WishList: FC<{}> = (): ReactElement => {
  ...
  const { serverError, apiData, isLoading } = UseAppQuery<WishListRes>(`/wishlist/${userId}`, memoizedParamData, {total: 0, wishList: []}, refresh)
  ...
  const handleDateAdd=async()=>{
    ...
    const wishlistData = await apiPostRequest("wishlist/add", postData);
    setRefresh(!refresh);
  }
  return (
